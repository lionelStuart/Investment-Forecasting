...
investment-forecasting ingest capital-flow --db data/investment_forecasting.sqlite3 --scope stock --asset-codes 600519 --max-days 20
investment-forecasting ingest fund-holdings --db data/investment_forecasting.sqlite3 --fund-codes 000001 --year 2024
...
每日流程的手机通知可以通过环境变量默认启用，命令行显式参数优先：

```bash
export INVESTMENT_FORECASTING_NOTIFY_RECIPIENT_KEY=owner_phone
export INVESTMENT_FORECASTING_NOTIFICATION_CHANNEL=imessage
export INVESTMENT_FORECASTING_NOTIFICATION_DRY_RUN=false
```
...
- 后续目标：继续深化 Tushare、真实行业分类、基金持仓和资金流，形成更稳定的全量资产池；当前已具备可选 Tushare 行情、确定性主题分类、主题配置页、资金流观测的 provider-neutral 入库/市场页查看/每日建议/Jarvis 证据链能力，以及基金持仓入库、基金页查看和持仓主题穿透能力。
...
调度、通信、专家团和 Jarvis 相关的大改动必须逐项通过回归验收清单：

- 清单文件：`repo/tasks/TASK-098-scheduler-communication-regression-checklist.md`
- 适用范围：scheduler、LaunchAgent、Tushare/AKShare 环境、市场/资讯增量、专家 T 日任务、Jarvis T+1 日报、iMessage/短讯、WebUI 任务健康。
- 验收规则：只要改动触及上述范围，PR/提交前必须逐项核对清单；任何一项失败，都不能视为本轮需求完成。
...
- 支持组合优化：资产相关性、目标波动率、风险预算、再平衡；当前每日建议已包含目标波动率和相关性风险预算证据。
